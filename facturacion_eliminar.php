...
$st=$pdo->prepare("SELECT estado FROM fe_documentos WHERE id=? AND empresa_id=? LIMIT 1");
$st->execute([$id,$empresa_id]);
$estado=$st->fetchColumn();
if(!$estado){ header("Location: facturacion.php"); exit; }
if($estado!=='PENDIENTE'){ header("Location: facturacion_ver.php?id=".$id); exit; }
$pdo->prepare("DELETE FROM fe_documentos WHERE id=? AND empresa_id=?")->execute([$id,$empresa_id]);
