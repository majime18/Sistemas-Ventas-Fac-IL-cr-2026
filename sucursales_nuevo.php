...
if($_SERVER['REQUEST_METHOD']==='POST'){
 try{
  $st=$pdo->prepare("INSERT INTO sucursales (empresa_id,nombre,estado) VALUES (?,?,1)");
  $st->execute([$_POST['empresa_id'],$_POST['nombre']]);
  header("Location: sucursales.php"); exit;
 }catch(Throwable $e){$msg="Error al guardar";}
}
?>
<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>Nueva Sucursal</title><style>
:root{
...
<div class="header"><div class="h1">Nueva Sucursal</div></div>
<?php if($msg):?><div class="alert err"><?=$msg?></div><?php endif;?>
<div class="card">
<form method="post">
<label>Empresa</label>
<select class="input" name="empresa_id"><?php foreach($emps as $e):?><option value="<?=$e['id']?>"><?=htmlspecialchars($e['nombre'])?></option><?php endforeach;?></select>
<label>Nombre</label><input class="input" name="nombre" required>
