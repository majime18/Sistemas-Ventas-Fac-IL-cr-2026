...
if ($empresa_id <= 0) {
    try {
        ...
        if ($uId > 0) {
            $stU = $pdo->prepare("SELECT empresa_id FROM usuarios WHERE id = ? LIMIT 1");
            $stU->execute([$uId]);
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(!$err){
    ...
    try{
      ...
      // Crédito -> CxC
      if($condicion==='CREDITO'){
        // CxC: esquema compatible con fac_il_cr.sql
        // cxc_documentos: (empresa_id, cliente_id, venta_id, fecha TIMESTAMP default, vencimiento DATE, vence DATE, total, saldo, estado)
        $venc = date('Y-m-d', strtotime("+$plazo days"));

        try{
          $stmtCxc = $pdo->prepare("
            INSERT INTO cxc_documentos
              (empresa_id, cliente_id, venta_id, vencimiento, vence, total, saldo, estado)
            VALUES
              (?,?,?,?,?,?,?, 'PENDIENTE')
          ");
          $stmtCxc->execute([$empresa_id, $cliente_id, $venta_id, $venc, $venc, $total, $total]);
        }catch(Throwable $e){
          $err = ($e instanceof PDOException) ? json_encode($e->errorInfo, JSON_UNESCAPED_UNICODE) : $e->getMessage();
          throw new Exception("Error guardando la venta: No se pudo crear CxC (cxc_documentos). SQLERR=".$err);
        }
      }
