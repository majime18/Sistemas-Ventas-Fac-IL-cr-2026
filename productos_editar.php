...
$st = $pdo->prepare("SELECT * FROM productos WHERE id=? AND empresa_id=? LIMIT 1");
$st->execute([$id,$empresa_id]);
$r = $st->fetch(PDO::FETCH_ASSOC);
if(!$r){ header("Location: productos.php"); exit; }
...
if(!$err){
  ...
  $st = $pdo->prepare("UPDATE productos SET codigo=?,codigo_barras=?,descripcion=?,categoria=?,cabys=?,unidad=?,costo=?,margen=?,margen_minimo=?,precio=?,impuesto_id=?,estado=? WHERE id=? AND empresa_id=?");
  $st->execute([$codigo,$codigo_barras===''?null:$codigo_barras,$descripcion,$categoria===''?null:$categoria,$cabys===''?null:$cabys,$unidad===''?null:$unidad,$costo,$margen,$margen_minimo,$precio,$impuesto_id,$estado,$id,$empresa_id]);
...
<title>Editar producto | FAC-IL-CR</title>
<style>
:root{
...
<body>
<div class="header">
  <div class="brand"><span class="dot"></span> FAC-IL-CR <span class="pill">Productos • Editar</span></div>
...
<div class="wrap">
  <div class="card">
    <div class="hd">
      <div>
        <div style="font-weight:1000;font-size:18px">Editar producto</div>
