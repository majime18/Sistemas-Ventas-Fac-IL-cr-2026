...
$st=$pdo->prepare("SELECT * FROM usuarios WHERE $where");
$st->execute($params);
...
$rol_id=(int)($_POST['rol_id'] ?? (int)($u['rol_id'] ?? 0));
$estado=(int)($_POST['estado'] ?? (int)($u['estado'] ?? 1));
$pass=($_POST['password'] ?? '');
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(!$err){
    // update base
    if($has_empresa){
      $pdo->prepare("UPDATE usuarios SET nombre=?, email=?, rol_id=?, estado=? WHERE id=? AND empresa_id=?")
          ->execute([$nombre,$email,$rol_id,$estado,$id,$empresa_id]);
    } else {
      $pdo->prepare("UPDATE usuarios SET nombre=?, email=?, rol_id=?, estado=? WHERE id=?")
          ->execute([$nombre,$email,$rol_id,$estado,$id]);
    }
    // optional password change
    if($pass!==''){
      if(strlen($pass)<6) $err="Contraseña mínimo 6 caracteres.";
      else {
        $hash=password_hash($pass,PASSWORD_DEFAULT);
        if($has_empresa){
          $pdo->prepare("UPDATE usuarios SET password=? WHERE id=? AND empresa_id=?")->execute([$hash,$id,$empresa_id]);
        } else {
          $pdo->prepare("UPDATE usuarios SET password=? WHERE id=?")->execute([$hash,$id]);
        }
      }
    }
    if(!$err){ header("Location: usuarios.php"); exit; }
  }
}
$is_me = ($id===$mi_id);
?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Editar Usuario</title><style>:root{--azul:#0b5ed7;--azul-metal:#084298;--amarillo:#ffc107;--amarillo-metal:#ffca2c;--fondo:#071225;--card:rgba(17,24,39,.78);--borde:rgba(255,255,255,.12);--txt:#e5e7eb;--muted:#a7b0c2;--ok:#22c55e;--bad:#ef4444;--info:#38bdf8}
...
<div class="header">
  <div class="brand"><span class="dot"></span> FAC-IL-CR <span class="pill">USUARIOS • EDITAR</span></div>
...
<div class="wrap"><div class="card">
  <div class="hd"><div>
    <div style="font-weight:1000;font-size:18px">Editar usuario</div>
    <div class="small">ID: <span class="mono"><?= (int)$id ?></span> <?= $is_me ? "• (tu usuario)" : "" ?></div>
  </div></div>
  <div class="bd">
    ...
    <form id="f" method="post" class="grid">
      ...
      <div style="grid-column:span 5"><div class="label">Nueva contraseña (opcional)</div>
        <input class="input mono" type="password" name="password" minlength="6" placeholder="Dejar en blanco para no cambiar">
        <div class="small">Mínimo 6 caracteres.</div>
      </div>

      <?php if($is_me): ?>
        <div style="grid-column:span 12" class="notice small">Nota: por seguridad, no podés desactivar tu propio usuario desde aquí.</div>
      <?php endif; ?>
    </form>
  </div>
</div></div>
