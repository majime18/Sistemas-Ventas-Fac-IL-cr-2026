  ...
  if(!$err){
    ...
    try{
      ...
      $insd = $pdo->prepare("
        ...
        (venta_id,producto_id,descripcion,cabys,cantidad,precio_unitario,descuento,impuesto_monto,impuesto_pct,total_linea)
        VALUES (?,?,?,?,?,?,?,?,?,?)
      ");
      ...
      $insMov = $pdo->prepare("
        ...
        (empresa_id,bodega_id,producto_id,tipo,referencia,documento_id,cantidad,costo_unitario,usuario_id,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,NOW())
      ");
      ...
      foreach($lineas as $l){
        ...
        if($tipo==='VENTA'){
          $updExist->execute([$l['cantidad'],$empresa_id,$bodega_id,$l['producto_id']]);
          $insMov->execute([
            $empresa_id,$bodega_id,$l['producto_id'],
            'SALIDA','VENTA',$venta_id,$l['cantidad'],0,$usuario_id
          ]);
        }
      }
      ...
      if($condicion==='CREDITO'){
        ...
        // Si tu tabla cxc_documentos usa nombres diferentes, ajustamos en 1 minuto.
        $pdo->prepare("
        ...
        ")->execute([$empresa_id,$cliente_id,$venta_id,$venc,$total,$total,'PENDIENTE']);
      }
      ...
      <form id="fventa" method="post" onsubmit="return beforeSubmit();">
        ...
        <div class="grid2" style="margin-top:8px">
          ...
          <div>
            ...
            <select name="medio_pago" class="input">
        ...
        <div class="grid2" style="margin-top:8px">
          <div>
            ...
            <input name="referencia_pago" class="input" placeholder="Autorización / SINPE / transferencia">
          </div>
          <div>
        ...
        <div class="kpis" style="margin-top:12px">
          ...
          <div class="kpi total"><div class="t">TOTAL A PAGAR</div><div class="v" id="kTot">₡0,00</div></div>
        </div>

        <div class="footerpay">
          <button type="button" class="btn good" onclick="setPago('EFECTIVO')">Efectivo</button>
          <button type="button" class="btn" onclick="setPago('TARJETA')">Tarjeta</button>
          <button type="button" class="btn warn" onclick="setPago('PAGO_MULTIPLE')">Pago múltiple</button>
        </div>
...
function syncBodega(fromSide){
  ...
  // refrescar grid y sugerencias (existencias)
  loadGrid();
  doSearch(true);
}
...
function setPago(tipo){
  // aquí solo cambia el select de medio_pago si querés; se maneja al guardar
  alert("Pago seleccionado: "+tipo+" (se puede completar con pantalla de cobro)");
}
...
function renderCart(){
  ...
  document.getElementById('kTot').textContent = crc(total);
...
function beforeSubmit(){
  ...
  if(condicionSel.value==='CREDITO' && Number(plazoInp.value||0)<=0){
    alert('Indicá plazo de crédito.');
    return false;
  }
  cartJson.value = JSON.stringify(cart);
  return true;
