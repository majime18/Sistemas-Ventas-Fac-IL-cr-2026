...
$st=$pdo->prepare("SELECT * FROM bodegas WHERE id=? AND empresa_id=? LIMIT 1");
$st->execute([$id,$empresa_id]);
$r=$st->fetch(PDO::FETCH_ASSOC);
if(!$r){ header("Location: bodegas.php"); exit; }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(!$err){
    $st=$pdo->prepare("UPDATE bodegas SET sucursal_id=?, nombre=?, estado=? WHERE id=? AND empresa_id=?");
    $st->execute([$sucursal_id,$nombre,$estado,$id,$empresa_id]);
...
<title>Editar bodega | FAC-IL-CR</title>
<style>
:root{
...
<div class="header">
  <div class="brand"><span class="dot"></span> FAC-IL-CR <span class="pill">Bodegas • Editar</span></div>
  <div class="actions">
...
<div class="wrap">
  <div class="card">
    <div class="hd"><div style="font-weight:1000;font-size:18px">Editar bodega</div></div>
    <div class="bd">
