...
if(!$r){ header("Location: contabilidad_cuentas.php"); exit; }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(!$err){
    $dup = $pdo->prepare("SELECT id FROM cont_cuentas WHERE empresa_id=? AND codigo=? AND id<>? LIMIT 1");
    $dup->execute([$empresa_id,$codigo,$id]);
    if($dup->fetch()) $err = "Ya existe otra cuenta con ese código.";
  }

  if(!$err){
    $antes = $r;
    $pdo->prepare("UPDATE cont_cuentas SET codigo=?, nombre=?, tipo=?, permite_mov=?, estado=? WHERE id=? AND empresa_id=?")
        ->execute([$codigo,$nombre,$tipo,$permite_mov,$estado,$id,$empresa_id]);
    $despues = ['codigo'=>$codigo,'nombre'=>$nombre,'tipo'=>$tipo,'permite_mov'=>$permite_mov,'estado'=>$estado];
    audit_log($pdo,$empresa_id,$usuario_id,'Contabilidad','EDITAR','cont_cuentas',$id,$antes,$despues);
    header("Location: contabilidad_cuentas.php"); exit;
  }

  $r = array_merge($r, ['codigo'=>$codigo,'nombre'=>$nombre,'tipo'=>$tipo,'permite_mov'=>$permite_mov,'estado'=>$estado]);
}
?>
...
<div class="header">
  <div class="brand"><span class="dot"></span> FAC-IL-CR <span class="pill">Catálogo • Editar</span></div>
  <div class="actions">
...
<div class="wrap">
  <div class="card">
    <div class="hd"><div style="font-weight:1000;font-size:18px">Editar cuenta</div></div>
    <div class="bd">
