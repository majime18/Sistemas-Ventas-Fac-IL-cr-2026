...
$st=$pdo->prepare("
  SELECT v.*, c.nombre cliente, c.identificacion, c.email cliente_email, c.telefono cliente_tel, c.direccion cliente_dir,
         u.nombre vendedor
  FROM ventas v
...
$det->execute([$id]); $det=$det->fetchAll(PDO::FETCH_ASSOC);

function crc($n){ return "₡".number_format((float)$n,2,'.',','); }
?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Venta #<?=$v['id']?> | FAC-IL-CR</title><style>
...
<div class="wrap">
  <div class="top">
    <div>
      <h1 class="h1">Venta #<?=$v['id']?> <span class="badge ok"><?=$v['estado']?></span></h1>
      <div class="sub">Cliente: <b><?=htmlspecialchars($v['cliente'] ?? 'N/D')?></b> • Vendedor: <?=htmlspecialchars($v['vendedor'] ?? 'N/D')?> • <?=htmlspecialchars($v['created_at'])?></div>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <a class="btn btn-ghost" href="ventas.php">← Volver</a>
      <a class="btn btn-warning" target="_blank" href="ventas_imprimir.php?id=<?=$v['id']?>">Imprimir</a>
      <a class="btn btn-primary" href="facturacion.php">Ir a Facturación</a>
    </div>
  </div>
      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="right"><?=number_format((float)$d['cantidad'],3,'.',',')?></td>
            <td class="right"><?=crc($d['precio_unitario'])?></td>
            <td class="right"><?=crc($d['impuesto_monto'])?></td>
            <td class="right" style="font-weight:1000"><?=crc($d['total_linea'])?></td>
          </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
      <?php if(!empty($v['observaciones'])): ?>
        <div class="alert" style="margin-top:12px"><b>Observaciones:</b> <?=htmlspecialchars($v['observaciones'])?></div>
      <?php endif; ?>
    </div>

    <div class="card">
      <div class="kpi"><div class="t">Subtotal</div><div class="v"><?=crc($v['subtotal'])?></div></div>
      <div class="kpi" style="margin-top:10px"><div class="t">Impuestos</div><div class="v"><?=crc($v['impuesto_total'])?></div></div>
      <div class="kpi" style="margin-top:10px;border-color:rgba(255,193,7,.35);background:linear-gradient(135deg,rgba(255,193,7,.14),rgba(2,6,23,.18))">
        <div class="t">Total</div><div class="v"><?=crc($v['total'])?></div>
      </div>
      <div class="alert" style="margin-top:14px">
        <b>Siguiente paso (Factura Electrónica):</b>
        <div class="muted" style="margin-top:6px">
          • En <b>Facturación</b>, creá el comprobante para esta venta y luego enviá con <code>facturacion_enviar_real.php</code>.<br>
          • Este módulo ya dejó el detalle listo en <code>ventas_detalle</code> para armar el XML.
        </div>
      </div>
    </div>
  </div>
