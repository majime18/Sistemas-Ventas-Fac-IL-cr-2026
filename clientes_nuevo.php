...
$tipo=$_POST['tipo_cliente'] ?? 'CONTADO';
$lim=(float)($_POST['limite_credito'] ?? 0);
$plazo=(int)($_POST['plazo_dias'] ?? 0);
$estado_credito=$_POST['estado_credito'] ?? 'ACTIVO';
$estado_in=$_POST['estado'] ?? (string)estado_activo_value($estado_numeric);

if($_SERVER['REQUEST_METHOD']==='POST'){
  if($nombre==='') $err="El nombre es obligatorio.";
  if(!$err){
    $cols=[]; $vals=[];
    if($has_empresa){ $cols[]="empresa_id"; $vals[]=$empresa_id; }
    $cols=array_merge($cols,["nombre","identificacion","email","telefono","direccion"]);
    $vals=array_merge($vals,[$nombre,$ident===''?null:$ident,$email===''?null:$email,$tel===''?null:$tel,$dir===''?null:$dir]);

    if($has_tipo){ $cols[]="tipo_cliente"; $vals[]=$tipo; }
    if($has_limite){ $cols[]="limite_credito"; $vals[]=$lim; }
    if($has_plazo){ $cols[]="plazo_dias"; $vals[]=$plazo; }
    if($has_estado_credito){ $cols[]="estado_credito"; $vals[]=$estado_credito; }
    if($has_estado){ $cols[]="estado"; $vals[]=($estado_numeric?(int)$estado_in:(string)$estado_in); }
...
<title>Nuevo Cliente</title><style>:root{--azul:#0b5ed7;--azul-metal:#084298;--amarillo:#ffc107;--amarillo-metal:#ffca2c;--fondo:#071225;--card:rgba(17,24,39,.78);--borde:rgba(255,255,255,.12);--txt:#e5e7eb;--muted:#a7b0c2;--ok:#22c55e;--bad:#ef4444;--info:#38bdf8}
...
<div class="header">
  <div class="brand"><span class="dot"></span> FAC-IL-CR <span class="pill">CLIENTES • NUEVO</span></div>
...
<div class="wrap"><div class="card">
  <div class="hd"><div><div style="font-weight:1000;font-size:18px">Crear cliente</div><div class="small">Control de crédito habilitado según columnas disponibles</div></div></div>
  <div class="bd">
    ...
    <form id="f" method="post" class="grid">
      ...
      <div style="grid-column:span 12" class="notice small">
        Tip: si el cliente es de crédito, define límite y plazo. Si es moroso o bloqueado, el sistema puede impedir ventas a crédito.
      </div>
    </form>
  </div>
</div></div>
